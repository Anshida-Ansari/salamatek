...
async function run() {
  try {
    ...
    for (const d of departmentsData) {
      // Find a department that matches the english name (case insensitive)
      const existing = await Department.findOne({ 'name.en': new RegExp('^' + d.nameEn + '$', 'i') });
      if (existing) {
        existing.subheading.en = d.subheading;
        existing.description.en = d.description;
        await existing.save();
        console.log('Updated: ' + d.nameEn);
      } else {
        console.log('Not found, skipping creation as per instructions (assuming already exist): ' + d.nameEn);
      }
    }
  } catch (err) {
    console.error(err);
