...
router
  ...
  .post(protect, authorize('admin', 'super_admin'), validate(createTestimonialSchema), createTestimonial);

router
  .route('/:id')
  .get(validate(getTestimonialSchema), getTestimonialById)
  .patch(protect, authorize('admin', 'super_admin'), validate(updateTestimonialSchema), updateTestimonial)
  .delete(protect, authorize('admin', 'super_admin'), validate(getTestimonialSchema), deleteTestimonial);
