...
export default function NewsPage() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const res = await fetchApi(`/api/news?limit=10&page=${page}&search=${encodeURIComponent(search)}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-serif font-bold text-brand-dark">News & Blog</h1>
                  ...
                  <tr key={news._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/admin/dashboard/news/${news._id}/edit`}
