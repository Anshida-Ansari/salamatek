...
function EmptyState({ search }: { search: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4">
      ...
      {!search && (
        <Link
          href="/admin/dashboard/health-packages/new"
...
export default function HealthPackagesPage() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const res = await fetchApi(`/api/health-packages?limit=10&page=${page}&search=${encodeURIComponent(search)}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-serif font-bold text-brand-dark">Health Packages</h1>
                  ...
                  <tr key={pkg._id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/admin/dashboard/health-packages/${pkg._id}/edit`}
