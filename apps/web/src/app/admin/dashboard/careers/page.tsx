    ...
    <div className="flex flex-col items-center justify-center py-16 px-4">
      ...
      <p className="text-sm text-gray-500 text-center max-w-xs mb-6">
        {search
          ...
          : 'Post your first job vacancy to attract talent.'}
      </p>
      {!search && (
        <Link
...
export default function CareersPage() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const res = await fetchApi(`/api/careers?limit=10&page=${page}&search=${encodeURIComponent(search)}`);
      setCareers(res.data ?? []);
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    setDeletingId(id);
    try {
      await fetchApi(`/api/careers/${id}`, { method: 'DELETE' });
      toast(`"${title}" was deleted successfully.`, 'success');
      setCareers((prev) => prev.filter((p) => p._id !== id));
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-serif font-bold text-brand-dark">Careers</h1>
          <p className="text-sm text-text-muted mt-1">
            {isLoading ? 'Loading...' : `${total} job${total !== 1 ? 's' : ''} total`}
          </p>
        </div>
        <Link
          href="/admin/dashboard/careers/new"
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50">
                  <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-6 py-3.5">Job Title</th>
                  <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-6 py-3.5 hidden sm:table-cell">Department</th>
                  ...
                  <tr key={career._id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-gray-900">{career.title.en}</p>
                          <p className="text-xs text-gray-400">/{career.slug}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 hidden sm:table-cell">
                      <span className="text-gray-700 font-medium">{career.department?.en || '—'}</span>
                    </td>
                    <td className="px-6 py-4">
                      <button
                        onClick={() => handleToggleActive(career._id, career.active, career.title.en)}
                        disabled={togglingId === career._id}
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => handleDelete(career._id, career.title.en)}
        ...
        {!isLoading && careers && careers.length > 0 && (
          <Pagination
            currentPage={page}
